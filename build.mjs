...
const [template, tokens, css, js, glossary, cases, meta, analysis, analysisCss, mathJs, chartsJs, analysisJs] = await Promise.all([
  ...
  read('analysis.css'), read('analysis-math.js'), read('analysis-charts.js'), read('analysis.js')
]);
...
const html = template.replace('/*__STYLES__*/', () => tokens + '\n' + css.replace(/^@import[^;]+;\s*/m, '') + '\n' + analysisCss)
  .replace('/*__DATA__*/', () => json).replace('/*__SCRIPT__*/', () => [mathJs,chartsJs,analysisJs,js].join('\n').replace(/<\/script/gi, '<\\/script'));
